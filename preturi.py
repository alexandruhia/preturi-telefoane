...
# Configurare pagină
st.set_page_config(page_title="ExpressCredit Pro - Fix Specs", layout="wide")
...
def load_excel_data():
    ...
    try:
        ...
        # CURĂȚARE COLOANE: eliminăm spațiile goale de la începutul sau sfârșitul numelui
...
def draw_label(row, fs, ls, l_sc, l_y, p_val, p_y, p_size, b_val, ag_val, bag_size, font_name):
    ...
    # 2. Specificații (Lista extinsă pentru a acoperi orice variație)
    # Verificăm dacă coloana există în rând, indiferent de litere mari/mici
    specs_to_show = ["Display", "OS", "Procesor", "Stocare", "RAM", "Camera principala", "Selfie", "Sanatate baterie", "Capacitate baterie"]
    
    y_curr = 260
    for s in specs_to_show:
        # Căutăm valoarea în rând (row are coloanele curățate deja de load_excel_data)
        valoare = row.get(s)
        if pd.notna(valoare) and str(valoare).strip() != "" and str(valoare).strip() != "-":
            txt_s = f"{s}: {valoare}"
            draw.text((80, y_curr), txt_s, fill="black", font=f_spec_bold)
            y_curr += ls
    ...
    try:
        ...
        logo_data = requests.get(l_url, timeout=3).content
...
if not df.empty:
    ...
    for i in range(3):
        with cols[i]:
            ...
            md = st.selectbox(f"Model {i+1}", sorted(df[df['Brand'] == br]['Model'].unique()), key=f"md{i}")
            row_data = df[(df['Brand'] == br) & (df['Model'] == md)].iloc[0]
            ...
            with st.expander("🎨 Ajustări", expanded=True):
                fn = st.selectbox("Font", ["Open Sans", "Roboto", "Montserrat", "Lato"], key=f"fn{i}")
            ...
            res_img = draw_label(row_data, fs, ls, lsc, ly, p_text, py, ps, b_v, a_v, bs, fn)
    ...
    if st.button("🚀 GENEREAZĂ PDF FINAL"):
        ...
        buf.seek(0)
        with open("f.png", "wb") as f: f.write(buf.read())
        pdf.image("f.png", x=5, y=5, w=287)
        st.download_button("💾 Descarcă PDF", pdf.output(dest='S').encode('latin-1'), "Etichete.pdf")
else:
    st.warning("Te rog verifică dacă fișierul Excel are coloanele: Brand, Model, Display etc.")
