...
st.set_page_config(page_title="ExpressCredit - Compact 70", layout="wide")
...
def creeaza_imagine_eticheta(row, t_size, f_size, sp, font_name, pret, b_cod, ag_val, bat_val, stoc_man, ram_man):
    # PÂNZA REDUSĂ LA 70% DIN INALTIME: 256x392 pixeli
    W, H = 256, 392 
...
for i in range(3):
    with col_labels[i]:
        ...
        with st.expander("Font & Spațiere"):
            ts = st.number_input("Mărime Titlu", 2, 500, 18, key=f"ts_{i}")
            fs = st.number_input("Mărime Specificații", 2, 500, 9, key=f"fs_{i}")
            ss = st.slider("Spațiere", 2, 500, 11, key=f"ss_{i}")
